...
void setupWebserver() {
  ...
  server.on("/panel_set", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (request->hasParam("longAddress", true) && request->hasParam("label", true)) {
      ...
      if (!isSafeLabel(lbl)) {
        request->send(400, "text/plain", TXT_ERR_INVALID_LABEL);
        return;
      }
    ...
    if (request->hasParam("file")) {
      ...
      if (SPIFFS.exists(fileToDelete)) {
        if (SPIFFS.remove(fileToDelete)) {
          ...
          request->send(500, "text/plain", TXT_ERR_DELETE_FILE);
        }
      } else {
        request->send(404, "text/plain", TXT_ERR_FILE_NOTFOUND);
      }
    } else {
      request->send(400, "text/plain", TXT_ERR_MISSING_PARAM);
    }
  });
  ...
  server.on("/reboot", HTTP_POST, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain", TXT_REBOOTING);
  ...
  server.on("/save", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    WebSerial.println(TXT_SAVED_VIA_ENDPOINT);
    request->redirect("/debug");
  });

  // 404-Behandlung
  server.onNotFound([](AsyncWebServerRequest *request) {
    request->send(404, "text/plain", TXT_404);
  });
}
...
String generateFileListHTML() {
  ...
  html += "<nav>"
    "<a href='/'>" TXT_NAV_HOME "</a>"
    "<a href='/debug'>" TXT_NAV_DEBUG "</a>"
    "<a href='/panels'>" TXT_NAV_PANELS "</a>"
    "<a href='/spiffs'>" TXT_NAV_SPIFFS "</a>"
    "<a href='#' class='danger' onclick=\"if(confirm('" TXT_CONFIRM_REBOOT "'))fetch('/reboot',{method:'POST'}).then(()=>setTimeout(()=>location.reload(),4000))\">" TXT_NAV_REBOOT "</a>"
    "</nav>";

  // Dateitabelle
  html += "<div class='card'>";
  html += "<h2>" TXT_SPIFFS_TITLE "</h2>";
  html += "<table><thead><tr><th>" TXT_SPIFFS_COL_NAME "</th><th>" TXT_SPIFFS_COL_SIZE "</th><th></th></tr></thead><tbody>";
  ...
  while (file) {
    ...
    html += "<td style='white-space:nowrap'>"
            "<a class='btn btn-dl' href='/download?file=" + urlEncode(fname) + "'>" TXT_SPIFFS_DOWNLOAD "</a> "
            "<a class='btn btn-del' href='/delete?file=" + urlEncode(fname) + "' "
            "onclick=\"return confirm('&quot;" + fnameEsc + "&quot; " TXT_SPIFFS_CONFIRM_DEL "')\">" TXT_SPIFFS_DELETE "</a>";
    html += "</td></tr>";
    file = root.openNextFile();
  }
  if (!hasFiles) html += "<tr><td colspan='3' style='color:var(--muted);text-align:center;padding:20px'>" TXT_SPIFFS_NOFILES "</td></tr>";
  html += "</tbody></table></div>";

  // Speicher
  html += "<div class='card'>";
  html += "<h2>" TXT_SPIFFS_STORAGE "</h2>";
  html += "<div class='bar-wrap'><div class='bar' style='width:" + String(usedPct, 0) + "%'></div></div>";
  html += "<div class='stat-row'>";
  html += "<span><b>" + String(usedPct, 0) + "%</b> " TXT_SPIFFS_USED "</span>";
  html += "<span><b>" + String(usedKB, 1) + " KB</b> / " + String(totalKB, 1) + " KB</span>";
  html += "<span>" TXT_SPIFFS_FREE " <b>" + String(freeBytes / 1024.0, 1) + " KB</b></span>";
  html += "<span>" TXT_SPIFFS_FREE_RAM " <b>" + String(freeHeap / 1024) + " KB</b></span>";
  html += "</div></div>";

  // Upload
  html += "<div class='card'>";
  html += "<h2>" TXT_SPIFFS_UPLOAD_H "</h2>";
  html += "<form method='POST' action='/save_upload' enctype='multipart/form-data'>";
  html += "<label>" TXT_SPIFFS_UPLOAD_LBL "</label>";
  html += "<input type='file' name='upload'>";
  html += "<button class='btn-up' type='submit'>" TXT_SPIFFS_UPLOAD_BTN "</button>";
  html += "</form></div>";
...
void handlePanelsPage() {
  server.on("/panels", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    h += "<div class='card'>";
    h += "<h2>" TXT_PANELS_HEADING " (&nbsp;" + String(NodeTable_count) + "&nbsp; " TXT_PANELS_NODES ")</h2>";
    h += "<p class='note' style='margin-bottom:12px'>" TXT_PANELS_DESC1 " "
         "<code style='color:var(--blue)'>panel</code> " TXT_PANELS_DESC2 "</p>";

    if (NodeTable_count == 0) {
      h += "<p style='color:var(--muted);padding:16px'>" TXT_PANELS_EMPTY "</p>";
    } else {
      h += "<table><thead><tr><th>#</th><th>" TXT_PANELS_COL_LONGADDR "</th><th>" TXT_PANELS_COL_ADDR "</th><th>" TXT_PANELS_COL_CURRENT "</th><th>" TXT_PANELS_COL_SET "</th><th></th></tr></thead><tbody>";
      for (int i = 0; i < NodeTable_count; i++) {
        ...
        if (lbl != "") {
          ...
          h += "<td class='lbl-empty'>" TXT_PANELS_UNASSIGNED "</td>";
        }
        // Formular festlegen
        h += "<td><form method='POST' action='/panel_set' style='display:flex;gap:6px;align-items:center'>"
             ...
             "<input type='text' name='label' placeholder='" TXT_PANELS_PLACEHOLDER "' value='" + lbl + "' maxlength='10'>"
             "<button class='btn btn-save' type='submit'>" TXT_PANELS_SAVE "</button>"
             "</form></td>";
        // Löschen
        if (lbl != "") {
          h += "<td><a class='btn btn-del' href='/panel_delete?longAddress=" + urlEncode(la) + "' "
               "onclick=\"return confirm('" TXT_PANELS_CONFIRM_REM " &quot;" + lbl + "&quot; " TXT_PANELS_CONFIRM_REM2 "')\">" TXT_PANELS_REMOVE "</a></td>";
        } else {
          h += "<td></td>";
        }
        h += "</tr>";
    ...
    h += "<div class='card'>";
    h += "<h2>" TXT_PANELS_MAPPING_H " (&nbsp;" + String(panelMap_count) + "&nbsp; " TXT_PANELS_ENTRIES ")</h2>";
    if (panelMap_count == 0) {
      h += "<p style='color:var(--muted);padding:8px'>" TXT_PANELS_NOMAPPING "</p>";
    } else {
      h += "<table><thead><tr><th>" TXT_PANELS_COL_LABEL "</th><th>" TXT_PANELS_COL_LONGADDR "</th><th></th></tr></thead><tbody>";
      for (int i = 0; i < panelMap_count; i++) {
        ...
        h += "<td><a class='btn btn-del' href='/panel_delete?longAddress=" + urlEncode(panelMap[i].longAddress) + "' "
             "onclick=\"return confirm('" TXT_PANELS_CONFIRM_REM3 "')\">×</a></td>";
        h += "</tr>";
      }
      h += "</tbody></table>";
    }
...
void handleDebugPage() {
  server.on("/debug", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    // Gerätetabelle
    h += "<div class='card'>";
    h += "<h2>" TXT_DEBUG_DEVICES_H " (&nbsp;" + String(deviceCount) + "&nbsp;)</h2>";
    h += "<table><thead><tr>"
      "<th>#</th><th>" TXT_DEBUG_COL_PANEL "</th><th>" TXT_DEBUG_COL_NODEID "</th><th>" TXT_PANELS_COL_ADDR "</th>"
      "<th>Vin</th><th>Vout</th><th>Duty</th>"
      "<th>" TXT_DEBUG_COL_STROM "</th><th>" TXT_DEBUG_COL_WATT "</th><th>" TXT_DEBUG_COL_TEMP "</th>"
      "<th>" TXT_DEBUG_COL_SLOT "</th><th>RSSI</th><th>Barcode</th>"
      "</tr></thead><tbody>";
    ...
    // NodeTable-Tabelle
    h += "<div class='card'>";
    h += "<h2>" TXT_DEBUG_NODETABLE_H " (&nbsp;" + String(NodeTable_count) + "&nbsp;)</h2>";
    if (NodeTable_changed) {
      h += "<div class='warn-box'>" TXT_DEBUG_UNSAVED "</div>";
    }
    h += "<table><thead><tr><th>#</th><th>" TXT_PANELS_COL_ADDR "</th><th>" TXT_PANELS_COL_LONGADDR "</th><th>Checksum</th></tr></thead><tbody>";
    for (int i = 0; i < NodeTable_count; i++) {
      h += "<tr>";
    ...
    h += "</tbody></table>";
    h += "<br><a class='btn-save' href='/save'>" TXT_DEBUG_SAVE_NOW "</a>";
    h += "</div>";
    ...
    [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
      ...
      if (!index) {
        ...
        for (size_t ci = 0; ci < filename.length(); ci++) {
          ...
          if (!isalnum(c) && c != '-' && c != '_' && c != '.') {
            Serial.println(TXT_ERR_INVALID_FNAME);
            return;
          }
        }
        if (filename.length() > 64) {
          Serial.println(TXT_ERR_FNAME_TOOLONG);
          return;
        }
      ...
      if (!spaceChecked) {
        if (SPIFFS.totalBytes() - SPIFFS.usedBytes() < len) {
          Serial.println(TXT_ERR_NO_SPACE);
